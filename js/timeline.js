...
galleryItems.forEach(item => {
  item.addEventListener('click', function() {
    ...
    captionText.innerHTML = this.querySelector('.gallery-overlay span').textContent;
...
function setupImageUpload() {
  ...
  // Tạo nút thêm ảnh (có thể thêm vào giao diện sau)
  const addButton = document.createElement('button');
  addButton.textContent = 'Thêm ảnh';
  addButton.classList.add('btn-add-image');
  addButton.addEventListener('click', function() {
    fileInput.click();
  });
  
  // Có thể thêm nút này vào giao diện sau
  // document.querySelector('.gallery .container').appendChild(addButton);
}

// Khởi tạo chức năng tải ảnh
// setupImageUpload();
